...
import { resetUserPasswordApi } from '@/services/users';

// Reset de la contraseña de otro usuario (sin pedir la actual). En Fase 2 lo
// puede hacer cualquier usuario logueado; en Fase 3 se restringe por rol.
export function ResetPasswordModal({ user, onClose }) {
  ...
  const submit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await resetUserPasswordApi(user.id, next);
      setDone(true);
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-stone-200">
          <h3 className="text-lg font-semibold text-stone-800 font-serif flex items-center gap-2">
            <KeyRound className="w-4 h-4 text-teal-600" /> Resetear contraseña
          </h3>
          <button
            type="button"
            onClick={onClose}
          ...
          {done ? (
            ...
            <>
              <div className="text-xs text-stone-500 bg-stone-50 border border-stone-200 rounded-lg px-3 py-2">
                Vas a definir una nueva contraseña para <strong className="text-stone-800">{user.email}</strong>. Comunícasela por un canal seguro.
              </div>
